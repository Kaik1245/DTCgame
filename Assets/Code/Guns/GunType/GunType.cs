...
public class GunType : MonoBehaviour
{
    ...
    // Set Damage amount to 0 if it is not charge gun
    public void ShootBullet(BulletType bullet, Vector2 OriginPosition, float Rotation, float DamageAmount)
    {
        var BulletInstance = Instantiate(bullet, OriginPosition, Quaternion.Euler(0, 0, Rotation)).gameObject;
        if(bullet is ChargeBullet)
        {
            BulletInstance.GetComponent<ChargeBullet>().ActualDamageAmount = DamageAmount * BulletInstance.GetComponent<ChargeBullet>().MaxDamageAmount;
        }
    }
